...
################## pre-dispense excute ####################
python3 automatic_spotassay/ot2_execute/compiler/compile_predispense.py ${target_dir}/predispense_spotassay.py $num_spot $p300_start $p20_start
...
python3 automatic_spotassay/ot2_execute/compiler/recipe_spotassay.py ${target_dir}/OD620.csv $num_spot recipe_spotassay.csv
# execute compile_protocol_spotassay_biological-test.py (serial No4 is dH20 as negative control)
python3 automatic_spotassay/ot2_execute/compiler/compile_after_plate_reader.py ${target_dir}/spotassay.py $num_spot $num_plate $p300_second $p20_second $weight_plate1 $weight_plate2 $weight_plate3 $weight_plate4
